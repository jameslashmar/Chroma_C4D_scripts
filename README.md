...
It can also bring the XPresso editor forward and centre the view on the node, but **that is off by default (`FOCUS_XPRESSO_EDITOR`) because it cannot be done safely.** `CallCommand(13038)` is the only way to scroll a graph to its selection, and it dispatches to whichever manager is *active* — `OpenDialog` only makes the editor active when it actually opens it, so if your XPresso window is already open the command falls through to the **3D viewport and zooms that instead**. There is no way to ask C4D which manager is active, so the miss can't be guarded against. Turn it on if the XPresso editor is reliably your focused manager; leave it off otherwise. Selecting the nodes always happens either way.
